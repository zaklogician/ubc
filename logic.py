...
def mk_eqs(var_c_args, var_c_rets, c_imem, c_omem, min_stack_size):
    arg_regs = mk_var_list(
        syntax.arch.argument_registers, syntax.arch.word_type)
    ar0 = arg_regs[0]
    ...
    sregs = mk_stack_sequence(sp, syntax.arch.ptr_size,
                              st, syntax.arch.word_type, len(var_c_args) + 1)
    ...
    if len(var_c_rets) > len(syntax.arch.return_registers):
        ...
        out_eqs = list(zip(var_c_rets, [x for (x, _) in save_seq]))
        ...
        out_eqs = list(
            zip(var_c_rets, arg_regs[:len(syntax.arch.argument_registers)]))
        save_addrs = []
    ...
    asm_invs = [((vin, 'ASM_IN'), (vout, 'ASM_OUT'))
                for (vin, vout) in post_eqs]
...
def accumulator_closed_form(expr, xxx_todo_changeme2, add_mask=None):
    (nm, typ) = xxx_todo_changeme2
...
def pvalid_assertion1(xxx_todo_changeme3, xxx_todo_changeme4):
    ...
    """
    (typ, k, p, pv) = xxx_todo_changeme3
    (typ2, k2, p2, pv2) = xxx_todo_changeme4
    offs1 = mk_minus(p, p2)
...
def pvalid_assertion2(xxx_todo_changeme5, xxx_todo_changeme6):
    """second pointer validity assertion: implication.
    pvalid1 & strictly-contained --> pvalid2
    """
    (typ, k, p, pv) = xxx_todo_changeme5
    (typ2, k2, p2, pv2) = xxx_todo_changeme6
...
def sym_distinct_assertion(xxx_todo_changeme7, xxx_todo_changeme8):
    (typ, p, pv) = xxx_todo_changeme7
    (start, end) = xxx_todo_changeme8
    out1 = mk_less(mk_plus(p, syntax.arch.mk_word(
        typ.size() - 1)), syntax.arch.mk_word(start))
...
def array_typ_size(xxx_todo_changeme9):
    (kind, el_typ, num, _) = xxx_todo_changeme9
...
def get_styp_condition_inner2(inner_typ, outer_typ):
    if inner_typ[0] == 'Array' and outer_typ[0] == 'Array':
        ...
        conds = [(get_styp_condition_inner1(inner_typ,
                 ...
                 for (_, offs2, sf_typ)
                 in structs[outer_typ[1].name].fields.values()]
...
def mk_array_size_ineq(typ, num, p):
    ...
    size_lim = ((2 ** syntax.arch.word_size) -
                syntax.arch.ptr_size) / typ.size()
...
def compute_var_deps(nodes, outputs, preds, override_lvals_rvals={},
    ...
    while visit:
        ...
        if trace and n in trace:
            ...
            printout('  %s, %s, %s, %s' %
                     (len(vs), len(cont_vs), len(lvals), len(rvals)))
        var_deps[n] = vs
...
def compute_loop_var_analysis(p, var_deps, n, override_nodes=None):
    ...
    vca = [(syntax.mk_var(nm, typ), data)
           for ((nm, typ), data) in list(vca.items())]
    return vca
...
def lv_expr(expr, env):
    ...
    def mk_offs(vals): return syntax.adjust_op_vals(expr, vals)
    if None in rs:
        return (None, None, None, None)
...
def linear_series_exprs(p, loop, va):
    ...
    def lv_merge(xxx_todo_changeme, xxx_todo_changeme1):
        (v1, lv1, offs1, oset1) = xxx_todo_changeme
        (v2, lv2, offs2, oset2) = xxx_todo_changeme1
        if v1 != v2:
            return (None, None, None, None)
...
def tm_with_word_list(xs):
    if xs:
        return foldr1(mk_plus, list(map(syntax.arch.mk_word, xs)))
    else:
        return mk_uminus(syntax.arch.mk_word(0))


def mk_eq_selective_wrapper(v, xxx_todo_changeme10):
    # this is a huge hack, but we need to put these lists somewhere
    (xs, ys) = xxx_todo_changeme10
