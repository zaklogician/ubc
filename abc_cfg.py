...
def loop_target_sorting_key(target: source.ExprVarT[source.ProgVarName | nip.GuardVarName]) -> tuple[int, str]:
...
def compute_loops(nodes: Mapping[source.NodeName, source.Node[source.ProgVarName | nip.GuardVarName]], cfg: CFG) -> Mapping[source.LoopHeaderName, source.Loop[source.ProgVarName | nip.GuardVarName]]:
    """ Map from loop header to source.Loop
    """
    assert_single_loop_header_per_loop(cfg)

    loops: dict[source.LoopHeaderName,
                source.Loop[source.ProgVarName | nip.GuardVarName]] = {}
